...
<template>
  <Transition name="fade">
    <div v-if="show"
      class="fixed top-0 left-0 w-screen h-screen bg-black/75 flex justify-center items-center z-100 backdrop-blur-xs tablet:p-4"
      @click.self="emit('close')">
      <div
        class="relative bg-white w-screen h-screen shadow-lg overflow-hidden flex flex-col pt-20 tablet:p-0 tablet:max-h-212.5 tablet:max-w-275 tablet:w-[95vw] tablet:h-[85vh] tablet:rounded-3xl">
        ...
        <button
          class="absolute top-6 right-7 bg-transparent border-none text-2xl text-gray-500 cursor-pointer z-10 transition duration-300 hover:text-evergreen hover:scale-110 hidden tablet:block"
        ...
        <div
          ...
          <div class="flex flex-col gap-5 flex-1 min-w-0">
            <h2
              class="text-[22px] pr-0 text-center tablet:text-start tablet:text-[28px] font-bold text-evergreen m-0 tablet:pr-10">
              {{ review?.title || 'Titolo non disponibile' }}
            </h2>

            <div
              class="flex items-center gap-4 flex-wrap text-sm pb-4 border-b border-gray-700 justify-center tablet:justify-start">
            ...
            <button @click="emit('close')"
              class="mt-auto w-100 max-w-full bg-mint-leaf text-white p-4 rounded-[14px] font-bold text-[16px] border-none cursor-pointer shadow-md transition duration-300 text-center hover:bg-evergreen hover:shadow-lg hover:-translate-y-0.5">Chiudi
              Lettura</button>
          </div>
        </div>
      </div>
