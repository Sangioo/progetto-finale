<template>
  <div
    class="m-4 rounded-xl mobilel:max-w-full mobilel:my-8 mobilel:mx-4 h-[70vh] flex laptop:max-w-313 laptop:my-8 laptop:mx-auto laptop:h-[75vh] laptop:rounded-2xl border border-gray-300 shadow-sm overflow-hidden relative">
    <BasePopup :show="isPopupOpen" title="Errore" :content="popupMessage" @close="isPopupOpen = false"
      @action="isPopupOpen = false" />
    <div class="border-r-0 flex flex-1 flex-col h-full tablet:border-r tablet:border-r-gray-300">
      <header class="p-4 mobilel:py-5 mobilel:px-6 border-b border-b-gray-300 flex justify-between items-center">
        <div>
          <h1 class="text-2xl font-extrabold uppercase tracking-wide">Stanza: <span class="text-mint-leaf"
              id="title">chat</span></h1>
          <p class="mt-1 text-sm">Goditi il film e chatta con la stanza</p>
        </div>
        <NuxtLink to="/"
      ...
      <div class="p-4 mobilel:p-6 flex flex-1 overflow-y-auto flex-col transition-all" ref="messagesAreaRef">
        ...
        <div v-else class="flex flex-col gap-5">
          <div v-for="(msg, index) in messages" :key="index"
            class="max-w-full mobilel:max-w-[75%] py-4 px-5 rounded-2xl border border-gray-300 shadow-sm relative transition-all"
            :class="{
              'is-spoiler': msg.spoiler == 1 && !msg.revealed,
              'self-end rounded-br-xs bg-mint-leaf/30': msg.username === currentUsername,
              'self-start rounded-bl-xs': msg.username !== currentUsername
            }">
            <div class="flex items-center gap-2 mb-2 text-xs">
              <span class="font-bold text-evergreen">
                {{ msg.username == currentUsername ? 'Tu' : msg.username }}
              </span>
              <span class="text-xs">{{ formatTime(msg.timestamp) }}</span>
            </div>

            <p v-if="!msg.spoiler" class="break-normal">
              {{ msg.message }}
            </p>

            <div v-else class="relative">
              <p class="break-normal" :class="{ 'blur-sm select-none pointer-events-none opacity-80': !msg.revealed }">
                {{ msg.message }}
              </p>
      ...
      <footer class="p-4 mobilel:py-5 mobilel:px-6 border-t border-gray-300">
        <form @submit.prevent="sendMessage" class="flex flex-col gap-4">
          <input v-model="newMessage" type="text" placeholder="Scrivi un messaggio (max 500 caratteri)..."
            maxlength="500"
            class="border border-gray-300 py-4 px-5 rounded-2xl outline-none shadow-sm transition-all focus:border-mint-leaf focus:shadow-lg" />
          ...
          <div
            ...
            <div class="flex justify-between mobilel:justify-normal items-center gap-4">
              <label class="flex items-center gap-2 cursor-pointer font-medium">
                <input v-model="sendAsSpoiler" type="checkbox" class="accent-mint-leaf cursor-pointer w-4 h-4" />
                <span>Spoiler</span>
              </label>

              <button type="button" @click="isSidebarOpen = true"
                class="flex tablet:hidden border border-gray-300 py-2 px-4 rounded-2xl cursor-pointer font-semibold items-center gap-2 transition-all"
                title="Mostra spettatori">
                👥 <span class="bg-mint-leaf text-white text-xs py-px px-1 rounded-lg font-semibold">{{
                  Object.keys(activeUsers).length }}</span>
              </button>
            </div>

            <button type="submit"
              class="bg-mint-leaf text-white py-2 px-7 rounded-2xl font-bold shadow-sm cursor-pointer transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:shadow-none"
    ...
    <aside
      class="fixed tablet:relative top-16 tablet:top-0 right-0 bottom-0 w-screen h-screen z-2 tablet:w-65 bg-white p-6 transition-all tablet:flex tablet:flex-col"
      :class="{ 'hidden': !isSidebarOpen, 'flex flex-col': isSidebarOpen }">
      <div class="flex justify-between items-center mb-5 border-b border-b-gray-300 pb-3">
        <h3 class="font-extrabold uppercase tracking-wide">Spettatori Attivi</h3>
        <button @click="isSidebarOpen = false"
          class="block tablet:hidden bg-transparent text-lg cursor-pointer p-1">✕</button>
      </div>
      ...
      <div class="flex flex-1 overflow-y-auto flex-col gap-2">
        <div v-for="(joinedAt, username) in activeUsers" :key="username"
          ...
          <div class="flex flex-col">
            ...
            <span class="text-xs">Entrato alle {{ formatTime(joinedAt) }}</span>
          </div>
        </div>
      </div>
    </aside>
